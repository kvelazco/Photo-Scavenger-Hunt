...
class HuntDetailViewController: UIViewController {
    ...
    /// Configure UI for the given Hunt
    private func updateUI() {

        self.title = hunt.title
        
        completedLabel.text = hunt.title
        
        descriptionLabel.text = hunt.description

        let completedImage = UIImage(systemName: hunt.isComplete ? "circle.inset.filled" : "circle")

        completedImageView.image = completedImage?.withRenderingMode(.alwaysTemplate)

        let color: UIColor = hunt.isComplete ? .systemOrange : .tertiaryLabel
    ...
    func updateMapView() {
        ...
        // Make sure the hunt has image location.
    ...
    func picker(_ picker: PHPickerViewController, didFinishPicking results: [PHPickerResult]) {
        ...
        provider.loadObject(ofClass: UIImage.self) { [weak self] object, error in
            ...
            DispatchQueue.main.async { [weak self] in
                ...
                // Set the picked image and location on the hunt
                self?.hunt.set(image, with: location)

                // Update the UI since we've updated the hunt
                self?.updateUI()
...
extension HuntDetailViewController: MKMapViewDelegate {
    ...
    func mapView(_ mapView: MKMapView, viewFor annotation: MKAnnotation) -> MKAnnotationView? {
        ...
        // Configure the annotation view, passing in the hunt's image.
...
extension HuntDetailViewController {
    ...
    func presentGoToSettingsAlert() {
        let alertController = UIAlertController (
            ...
            message: "In order to post a photo to complete a hunt, we need access to your photo library. You can allow access in Settings",
